...
def imap_connect(server: str, user: str, password: str, max_retries: int, retry_delay: int) -> imaplib.IMAP4_SSL:
    ...
    while attempts < max_retries:
        try:
            ...
            auth_keywords = [
                "AUTHENTICATIONFAILED", "INVALID CREDENTIALS", "INVALID CREDENTIAL",
                "НЕВЕРНЫЕ УЧЕТНЫЕ ДАННЫЕ", "ОШИБКА АУТЕНТИФИКАЦИИ", "АУТЕНТИФИКАЦИЯ",
            ...
            if any(keyword in error_str for keyword in auth_keywords):
                ...
                raise ConnectionError(f"IMAP authentication failed: {error_msg}")
            ...
            raise ConnectionError(f"IMAP encoding error (likely authentication failure with non-ASCII characters): {repr(e)}")
...
class IMAPClient:
    ...
    def connect(self) -> imaplib.IMAP4_SSL:
        """Connect to IMAP server with retry logic."""
        self._mail = imap_connect(self.server, self.user, self.password, self.max_retries, self.retry_delay)
        return self._mail
    ...
    def fetch_uid(self, msg_id: bytes) -> Optional[str]:
        ...
        if not self._mail:
            raise ConnectionError("Not connected to IMAP server")
        import re
        status, meta = self._mail.fetch(msg_id, "(UID RFC822.SIZE BODYSTRUCTURE)")
    ...
    def fetch_headers(self, msg_id: bytes) -> Optional[email.message.Message]:
        ...
        if not self._mail:
            raise ConnectionError("Not connected to IMAP server")
        status, header_data = self._mail.fetch(msg_id, "(BODY.PEEK[HEADER.FIELDS (FROM SUBJECT DATE)])")
    ...
    def fetch_message(self, msg_id: bytes) -> Optional[email.message.Message]:
        ...
        if not self._mail:
            raise ConnectionError("Not connected to IMAP server")
        status, full_data = self._mail.fetch(msg_id, "(RFC822)")
