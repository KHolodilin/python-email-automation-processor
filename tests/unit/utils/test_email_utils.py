...
class TestEmailUtils(unittest.TestCase):
    ...
    def test_parse_email_date_value_error(self):
        ...
        with patch("email_processor.utils.email_utils.parsedate_to_datetime", side_effect=ValueError("Invalid date")):
            result = parse_email_date("invalid")
            self.assertIsNone(result)
    
    def test_parse_email_date_type_error(self):
        """Test email date parsing with TypeError."""
        with patch("email_processor.utils.email_utils.parsedate_to_datetime", side_effect=TypeError("Invalid type")):
            result = parse_email_date("invalid")
            self.assertIsNone(result)
    
    def test_parse_email_date_unexpected_error(self):
        """Test email date parsing with unexpected error."""
        with patch("email_processor.utils.email_utils.parsedate_to_datetime", side_effect=Exception("Unexpected")):
            result = parse_email_date("invalid")
            self.assertIsNone(result)
